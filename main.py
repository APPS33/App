...
class ToDoApp(App):
    def build(self):
        ...
        self.root.add_widget(
            Label(text="قائمة مهامي", font_size=30, color=(0.2, 0.6, 1, 1), bold=True)
        )

        # منطقة الإدخال
        input_area = BoxLayout(size_hint_y=None, height=50, spacing=10)
        self.task_input = TextInput(
            hint_text="اكتب مهمة جديدة...", multiline=False, halign="right"
        )
        add_btn = Button(
            text="إضافة", background_color=(0.2, 0.8, 0.2, 1), size_hint_x=0.3
        )
    ...
    def add_task(self, instance):
        if self.task_input.text:
            ...
            del_btn = Button(
                text="X", size_hint_x=0.2, background_color=(1, 0.3, 0.3, 1)
            )
